...
int main(int argc, char *argv[])
{
    ...
    //printf("Proceso coche ejecutandose, soy el proceso %d de %d procesos\n",rank,nprocs);

    while (1)
    {   
        mensaje[0] = rank;       /* id del proceso */
        mensaje[1] = ENTRAR;     /* accion que realiza el coche, por defecto entrar */
        mensaje[2] = -100;      /* tipo de vehiculo */
        mensaje[3] = -100;
        /* vehiculo quiere entrar en el parking */
        MPI_Send(&mensaje,4,MPI_INT,0,COCHES,MPI_COMM_WORLD);
        ...
        if (respuesta[0]>=0)    
        {
            ...
            MPI_Send(&mensaje,4,MPI_INT,0,COCHES,MPI_COMM_WORLD);    /* envio mensaje de que quiero salir del parking */
